...
def updateVpcTable(tableName, data, status):
    ...
    try:
        ...
        response = table.update_item(Key={'VpcId': data['queryStringParameters']['VpcId']},
                                     AttributeUpdates={'CurrentStatus': {'Value': status, 'Action': 'PUT'},
                                                       'Node1VpnId': {'Value': data['queryStringParameters']['VpnN1'],
                                                                      'Action': 'PUT'},
                                                       'Node2VpnId': {'Value': data['queryStringParameters']['VpnN2'],
                                                                      'Action': 'PUT'},
                                                       'Region': {'Value': data['queryStringParameters']['Region'],
                                                                  'Action': 'PUT'}, 'IpSegment': {
                                             'Value': data['queryStringParameters']['IpSegment'], 'Action': 'PUT'}})
...
def updateBgpTunnelIpPool(bgpTableName, ipSegment):
    ...
    try:
        ...
        table.update_item(Key={'IpSegment': ipSegment},
            ...
            if response[0]['VpcCount'] > 0:
                table.update_item(Key={'PaGroupName': paGroupName},
                                  AttributeUpdates={'InUse': {'Value': 'YES', 'Action': 'PUT'},
                                                    'VpcCount': {'Value': value, 'Action': 'ADD'}})
            ...
            for item in response['Items']:
                if 'VpcId' in item:
                    if item['VgwAsn'] != vgwAsn:
                        ...
                        logger.info(
                            "VgwAsn are mis-matched, because user created the VGW and attached it to VPC, hence using the same VGW, updating previously allocated VgwAsn: {} with InUse=NO in Transit VgwAsn table".format(
                                item['VgwAsn']))
                        return
...
def lambda_handler(event, context):
    ...
    if config:
        paGroupInfo = getPaGroupInfo(config['TransitPaGroupInfo'], event['queryStringParameters']['PaGroupName'])
        if paGroupInfo:
            ...
            paVpnStatus = pan_vpn_generic.paGroupConfigureVpn(api_key, paGroupInfo, config['TransitVpnBucketName'],
                                                              event['queryStringParameters']['VpnN1'],
                                                              event['queryStringParameters']['VpnN2'])
            ...
            if paVpnStatus:
                ...
                updateVgwAsn(config['TransitVgwAsn'], event['queryStringParameters']['VpcId'],
                             event['queryStringParameters']['VgwAsn'])
                logger.info("Updated Table: {}".format(config['TransitVgwAsn']))
                data = {
                    'Action': 'VpnConfigured',
                    'VpcId': event['queryStringParameters']['VpcId'],
                    'PaGroupName': event['queryStringParameters']['PaGroupName'],
                ...
                updatePaGroup(config['TransitPaGroupInfo'], event['queryStringParameters']['PaGroupName'], -1)
                updateBgpTunnelIpPool(config['TransitBgpTunnelIpPool'], event['queryStringParameters']['IpSegment'])
            ...
            logger.error("No Item received from PaGroupInfo table with Key: {}".format(
                event['queryStringParameters']['PaGroupName']))
    else:
