...
from commonLambdaFunctions import createVpnConnectionUploadToS3, fetchFromSubscriberConfigTable, createVgwAttachToVpc, \
    createCgw, publishToSns, isVgwAttachedToVpc, checkCgw, deleteVgw
    ...
    try:
        ...
        if subscriberConfig:
            ...
            if not vgwId:
                ...
                vgwId = createVgwAttachToVpc(event['VpcId'], int(event['VgwAsn']), event['Region'],
                                             event['PaGroupName'])
            ...
            if vgwId: vpnId1 = createVpnConnectionUploadToS3(event['Region'], vgwId, cgwNode1Id, event['N1T1'],
                                                             event['N1T2'], vpn1Tag, event['TransitVpnBucketName'],
                                                             event['TransitAssumeRoleArn'])
            logger.info("VPN1 - {} is created for VPC - {} with PA-Group: {}".format(vpnId1, event['VpcId'],
                                                                                     event['PaGroupName']))
            # Crete VPN2 connection with Node2
            if vgwId: vpnId2 = createVpnConnectionUploadToS3(event['Region'], vgwId, cgwNode2Id, event['N2T1'],
                                                             event['N2T2'], vpn2Tag, event['TransitVpnBucketName'],
                                                             event['TransitAssumeRoleArn'])
            logger.info("VPN2 - {} is created for VPC - {} with PA-Group: {}".format(vpnId2, event['VpcId'],
                                                                                     event['PaGroupName']))
