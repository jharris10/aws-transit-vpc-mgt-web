...
def lambda_handler(event, context):
    ...
    try:
        ...
        # New code
        try:
            logger.info('Data object is {}'.format(data))
            ...
            if subscriberConfig:
                vgwId = False
                vgwData = isVgwAttachedToVpc(event['queryStringParameters']['VpcId'],
                ...
                if not vgwId:
                    ...
                    vgwId = createVgwAttachToVpc(data['VpcId'], int(data['VgwAsn']),
                                                 event['queryStringParameters']['Region'],
                                                 data['PaGroupName'])
                    logger.info("VGW - {} is created and attached to VPC - {}".format(vgwId, data['VpcId']))
                else:
                    logger.info(
                        "Using existing Vgw: {} for VPC: {} ".format(vgwId, event['queryStringParameters']['VpcId']))
                logger.info("Checking whether CGWs are already created or not")
                cgwIds = checkCgw(event['queryStringParameters']['Region'], paGroup['N1Eip'], paGroup['N2Eip'])
                if not cgwIds:
                    ...
                    cgwNode1Id = createCgw(paGroup['N1Eip'], str(paGroup['N1Asn']),
                                           event['queryStringParameters']['Region'], cgw1Tag)
                    logger.info(
                        "CGW - {} is created for VPC - {}".format(cgwNode1Id, event['queryStringParameters']['VpcId']))
                    # Create CGW2
                    cgw2Tag = paGroup['PaGroupName'] + '-N2'
                    cgwNode2Id = createCgw(paGroup['N2Eip'], str(paGroup['N2Asn']),
                                           event['queryStringParameters']['Region'], cgw2Tag)
                    logger.info(
                        "CGW - {} is created for VPC - {}".format(cgwNode1Id, event['queryStringParameters']['VpcId']))
                else:
                ...
                vpn1Tag = event['queryStringParameters']['VpcId'] + '-' + paGroup['PaGroupName'] + '-N1'
                vpn2Tag = event['queryStringParameters']['VpcId'] + '-' + paGroup['PaGroupName'] + '-N2'
                # Create VPN1 connection with Node1
                #
                if vgwId: vpnId1 = createVpnConnectionUploadToS3(event['queryStringParameters']['Region'], vgwId,
                                                                 cgwNode1Id, bgpIpPool['N1T1'],
                                                                 bgpIpPool['N1T2'], vpn1Tag,
                ...
                if vgwId: vpnId2 = createVpnConnectionUploadToS3(event['queryStringParameters']['Region'], vgwId,
                                                                 cgwNode2Id, bgpIpPool['N2T1'],
                                                                 bgpIpPool['N2T2'], vpn2Tag,
                                                                 transitConfig['TransitVpnBucketName'],
                                                                 transitConfig['TransitAssumeRoleArn'])
                ...
                #
                #
                # Forming an output to sent to The Browser
            ...
            if vgwId: deleteVgw(vgwId, event['queryStringParameters']['VpcId'],
                                event['queryStringParameters']['Region'])
            #
            # Create cleanup function for database functions putItemSubscriberLocalDb, updateVpcVpnTable, createCgw
            #
            data = {
                'Result': 'Failed',
